...
// Modified again by Robert Smallshire to be a standalone AVR library
...
uint8_t isRunning() {
    I2C::read(DS_ADDRESS, REG_ZERO, 1);
    uint8_t ss = I2C::receive();
    return !(ss>>7);
}

void setDatetime(const DateTime& dt) {
    I2C::writeBytes(DS_ADDRESS, REG_ZERO, 7,
    		bin2bcd(dt.second()),
    		bin2bcd(dt.minute()),
    		bin2bcd(dt.hour()),
    		bin2bcd(0),
...
DateTime getDatetime() {  
    uint8_t result = I2C::read(DS_ADDRESS, REG_ZERO, 7);
    if (result != 0) {
...
void setOutput(bool state) {
    ...
    I2C::write(uint8_t(DS_ADDRESS), uint8_t(7), control);
}
...
uint8_t readByte(uint8_t offset, uint8_t & data) {
    offset %= 56;
    offset += 8;
    uint8_t status = I2C::read((uint8_t)DS_ADDRESS, offset, (uint8_t)1);
    if (status != 0) {
	return status;
    }
    if (I2C::available() == 0) {
...
uint8_t writeByte(uint8_t offset, uint8_t data) {
    offset %= 56;
    offset += 8;
    return I2C::write(uint8_t(DS_ADDRESS), offset, data);
}
...
uint8_t readBytes(uint8_t offset, uint8_t* data, uint8_t numBytes)
{
    offset %= 56;
    offset += 8;
    return I2C::read(DS_ADDRESS, offset, numBytes, data);
}
...
uint8_t writeBytes(uint8_t offset, uint8_t* data, uint8_t numBytes)
{
    offset %= 56;
    offset += 8;
    return I2C::write(DS_ADDRESS, offset, data, numBytes);
}
*/
